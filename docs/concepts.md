...
- **Decoder**: The Transformer decoder autoregressively emits tokens for expressions in prefix order, conditioned on the encoder output and optional prompts. Since v26 the encoder output is a prefix of the decoder's own sequence (`<bos> <data> ... </data> <expression> ...`, bidirectional over the data block, causal over the expression); the v25 and earlier models read it through cross-attention instead (`decoder_data_mode`).
