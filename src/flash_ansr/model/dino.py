...
class DINOEncoder(nn.Module):
    def __init__(
        ...
        if self.encoder.output_dim != dino_hidden_dim:
            dino_head_input_dim = self.encoder.output_dim

        self.dino_head = nn.Sequential(
            nn.Linear(dino_head_input_dim, dino_hidden_dim),
            nn.GELU(),
            nn.Dropout(p=dino_dropout),
            nn.Linear(dino_hidden_dim, dino_output_dim)
        )
    ...
    @classmethod
    def from_config(cls, config: dict[str, Any] | str) -> "DINOEncoder":
        config_ = load_config(config)

        if "dino_encoder" in config_.keys():
            config_ = config_["dino_encoder"]
        ...
        return cls(
            ...
            dino_hidden_dim=config_["dino_hidden_dim"],
            dino_output_dim=config_["dino_output_dim"],
            dino_dropout=config_["dino_dropout"],
    ...
    def forward(self, data: torch.Tensor, data_attn_mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        representation = self.dino_head(representation)

        return representation

    @classmethod
    def load(cls, directory: str) -> tuple[dict[str, Any], "DINOEncoder"]:
        directory = substitute_root_path(directory)

        config_path = os.path.join(directory, 'dino_encoder.yaml')
