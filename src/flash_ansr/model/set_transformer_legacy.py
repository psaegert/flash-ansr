...
class LegacySAB(nn.Module):
    # https://github.com/juho-lee/set_transformer
    def __init__(self, input_size: int, output_size: int, n_heads: int, signed_softmax: bool = False, layer_norm: bool = False) -> None:
        super().__init__()
        self.mab = LegacyMAB(input_size, input_size, output_size, n_heads, signed_softmax, layer_norm)

    def forward(self, X: torch.Tensor) -> torch.Tensor:
        return self.mab(X, X)


class LegacyISAB(nn.Module):
    # https://github.com/juho-lee/set_transformer
    def __init__(self, input_size: int, output_size: int, n_heads: int, n_induce: int, signed_softmax: bool = False, layer_norm: bool = False) -> None:
        ...
        self.mab0 = LegacyMAB(output_size, input_size, output_size, n_heads, signed_softmax, layer_norm)
        self.mab1 = LegacyMAB(input_size, output_size, output_size, n_heads, signed_softmax, layer_norm)
...
class LegacyPMA(nn.Module):
    ...
    def __init__(self, size: int, n_heads: int, n_seeds: int, signed_softmax: bool = False, layer_norm: bool = False) -> None:
        ...
        self.mab = LegacyMAB(size, size, size, n_heads, signed_softmax, layer_norm)

    def forward(self, X: torch.Tensor) -> torch.Tensor:
        return self.mab(self.S.repeat(X.size(0), 1, 1), X)


class LegacySetTransformer(SetEncoder):
    # https://github.com/juho-lee/set_transformer
    def __init__(
            self,
        ...
        self.enc = nn.Sequential(
            LegacyISAB(input_embedding_size * input_dimension_size, hidden_size, n_heads, n_induce[0], signed_softmax, layer_norm),
            *[LegacyISAB(hidden_size, hidden_size, n_heads, n_induce[i + 1], signed_softmax, layer_norm) for i in range(n_enc_isab - 1)])

        self.dec = nn.Sequential(
            LegacyPMA(hidden_size, n_heads, n_seeds, signed_softmax, layer_norm),
            *[LegacySAB(hidden_size, hidden_size, n_heads, signed_softmax, layer_norm) for _ in range(n_dec_sab)],
