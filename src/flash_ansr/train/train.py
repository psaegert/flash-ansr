...
from flash_ansr.train.scheduler import BatchSizeScheduler, BatchSizeSchedulerFactory, LRSchedulerFactory
...
class Trainer():
    ...
    def from_config(cls, config: dict[str, Any] | str) -> "Trainer":
        ...
        print(f'Loading batch_size_scheduler with config {config_["batch_size_scheduler"]}')
        batch_size_scheduler = BatchSizeSchedulerFactory.get_scheduler(config_['batch_size_scheduler']['name'], **config_['batch_size_scheduler']['kwargs'] if 'kwargs' in config_['batch_size_scheduler'] else {})
        ...
        return cls(
            ...
            batch_size_scheduler=batch_size_scheduler,
    ...
    def run_from_config(
        ...
        return self.run(
            ...
            entity=entity,
            name=name,
            numeric_prediction_loss_weight=self.config['numeric_prediction_loss_weight'],
    ...
    def run(
        ...
        with wandb.init(config=wandb_config, project=project_name, entity=entity, name=name, mode=wandb_mode):  # type: ignore
            ...
            if validate_interval is not None:
                _ = self._validate(self.val_dataset, numeric_prediction_loss_weight=numeric_prediction_loss_weight, step=0, size=validate_size, batch_size=validate_batch_size, verbose=verbose)

            # Train the model
            for step, batch in enumerate(self.train_dataset.iterate(steps=steps, batch_size=self.batch_size_scheduler)):
                pbar.set_description("Training")

                # Train
                _ = self._train_batch(batch, numeric_prediction_loss_weight=numeric_prediction_loss_weight, step=step)

                pbar.update(1)

                if validate_interval is not None and ((step + 1) % validate_interval == 0 or step == steps - 1):
                    # Validate
                    _ = self._validate(self.val_dataset, numeric_prediction_loss_weight=numeric_prediction_loss_weight, step=(step + 1), size=validate_size, batch_size=validate_batch_size, verbose=verbose)
    ...
    def _train_batch(self, batch: dict[str, torch.Tensor], numeric_prediction_loss_weight: float, step: int) -> float:
        self.model.train()
        ...
        for acc_step in range(self.gradient_accumulation_steps):
            ...
            input_ids, x_tensor, y_tensor, labels, constants = FlashANSRDataset.collate_batch(micro_batch, device=self.model.device)
            ...
            loss = (ce_loss + numeric_prediction_loss_weight * num_loss) / (1 + numeric_prediction_loss_weight)
        ...
        self.batch_size_scheduler.step(step)

        return loss.item()

    def _validate(self, val_dataset: FlashANSRDataset, numeric_prediction_loss_weight: float, step: int, size: int | None = None, batch_size: int = 32, verbose: bool = False) -> float:
        self.model.eval()
        with torch.no_grad():
            val_ce_loss = 0.0
            val_num_loss = 0.0
            ...
            for batch in val_dataset.iterate(size=size, batch_size=batch_size):  # TODO: support both compiled dataset and non-compiled dataset that may use a custom batch size
                # Forward
                input_ids, x_tensor, y_tensor, labels, constants = FlashANSRDataset.collate_batch(batch, device=self.model.device)
