...
the global kill-switch is on AND the decode is MULTI-chunk (the resolved static batch < draws). These
...
def _resolve(stub, static_decode, draws=None, static_batch=None):
    return fam.FlashANSRModel._resolve_static_decode(
        stub, static_decode, draws=draws, static_batch=static_batch)
...
class TestExplicit(unittest.TestCase):
    def test_explicit_true_capable(self) -> None:
        self.assertTrue(_resolve(_stub(), True, draws=1024, static_batch=256))

    def test_explicit_true_incapable_warns_and_falls_back(self) -> None:
        with self.assertWarns(UserWarning):
            self.assertFalse(_resolve(_stub(capable=False), True, draws=1024, static_batch=256))

    def test_explicit_false(self) -> None:
        self.assertFalse(_resolve(_stub(), False, draws=1024, static_batch=64))


class TestRegimePredicate(unittest.TestCase):
    def test_multichunk_enables_static(self) -> None:
        # static_batch < draws -> multi-chunk -> static
        self.assertTrue(_resolve(_stub(), None, draws=1024, static_batch=256))
        self.assertTrue(_resolve(_stub(), None, draws=262144, static_batch=256))
        self.assertTrue(_resolve(_stub(), None, draws=512, static_batch=64))

    def test_singlechunk_stays_dynamic(self) -> None:
        # static_batch >= draws -> single-chunk -> dynamic (excludes the measured single-chunk losses)
        self.assertFalse(_resolve(_stub(), None, draws=64, static_batch=64))
        self.assertFalse(_resolve(_stub(), None, draws=512, static_batch=512))
        self.assertFalse(_resolve(_stub(), None, draws=64, static_batch=256))

    def test_unknown_batch_or_choices_is_dynamic(self) -> None:
        self.assertFalse(_resolve(_stub(), None, draws=1024, static_batch=None))  # CPU / direct caller
        self.assertFalse(_resolve(_stub(), None, draws=None, static_batch=256))

    def test_incapable_is_dynamic(self) -> None:
        self.assertFalse(_resolve(_stub(capable=False), None, draws=1024, static_batch=256))

    def test_kill_switch_off_forces_dynamic(self) -> None:
        with mock.patch.object(fam, "_DEPLOYED_STATIC_ENABLED", False):
            self.assertFalse(_resolve(_stub(), None, draws=1024, static_batch=256))
