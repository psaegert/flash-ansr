...
instead of failing::
    ...
    engine.simplify(['/', 'x1', '0'], form='explicit') -> ['*', 'float("inf")', 'x1']
    engine.simplify(['/', '0',  '0'], form='explicit') -> ['float("nan")']
    engine.simplify(['log', '0'],     form='explicit') -> ['float("-inf")']
...
class TestPremise(unittest.TestCase):
    ...
    def test_simplipy_still_folds_degenerate_expressions_to_non_finite(self) -> None:
        ...
        for expression, expected in [
            ...
            simplified = engine.simplify(list(expression), form='explicit')
